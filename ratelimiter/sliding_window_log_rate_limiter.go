...
type SlidingWindowLogRateLimiter struct {
	...
	mutex                    *sync.Mutex
	now                      func() time.Time // needed for unit testing.
}

func NewSlidingWindowLogRateLimiter(rateLimitCount int, rateWindowInMilliseconds time.Duration) *SlidingWindowLogRateLimiter {
...
// AllowRequest returns true if in the last past X milliseconds, there was less requests than the rate limit.
// Remove all the old requests from the map.
func (s *SlidingWindowLogRateLimiter) AllowRequest(userID string) bool {
	now := s.now()
	s.mutex.Lock()
...
func (s *SlidingWindowLogRateLimiter) getRequestsInTheWindowTime(requests []time.Time, now time.Time) []time.Time {
	newRequests := make([]time.Time, 0)
	for _, request := range requests {
		// Take into consideration just the requests inside the window time.
